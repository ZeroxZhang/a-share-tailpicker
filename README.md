...
`a-share-tailpicker` 是一个用于 A 股沪主板尾盘选股的 skill。它把 C 版尾盘交易方法论做成可重复运行的脚本流程：

- 只筛选默认 C 版沪主板 `60` 系列标的
- 排除科创板、创业板、北交所、ST、极端换手、涨跌停附近和弱流动性标的
- 使用分时形态、量价、资金代理、均线状态、板块共振和交叉验证进行评分
- 输出三层结果：正式可买、建议观察、市场说明
- 支持次日条件退出回测（保守成交模型 + 显式成本）

v4 起采用多源数据（iFinD 优先于 Kimi Code / AKShare 多端点 / 东财 HTTP）、富化缓存层、保守成交模型、组合风控与反过拟合护栏。
...
rm -rf ~/.claude/skills/a-share-tailpicker
git clone https://github.com/ZeroxZhang/a-share-tailpicker ~/.claude/skills/a-share-tailpicker
```

确认文件存在：

```bash
ls ~/.claude/skills/a-share-tailpicker
```
...
脚本使用 Python 3。live 数据路径使用多源 DataBackend（iFinD 优先于 Kimi Code / AKShare 多端点 / 东财 HTTP）。
...
- CLI 参数可用性
- v4 模块：保守成交模型（跳空/止盈 bar 收盘确认/成本含印花税）、统计（Wilson CI/回撤/Sharpe/归因）、组合风控（行业集中度/bear 收紧/halt 清仓）、`fundamentals_status` 封顶、标准量比回退
...
5. 次日严格执行条件退出策略（开盘≥+0.5%获利了结 / 开盘≤-0.5%止损 / 否则盘中+0.5%止盈或14:50前时间止损）。
...
Copy the repository contents into your local skills folder:
...
The script uses Python 3. The live data path uses the multi-source DataBackend (iFinD preferred on Kimi Code, AKShare multi-endpoint, Eastmoney HTTP).
...
5. Follow the next-day conditional exit strategy (open >=+0.5% take-profit / open <=-0.5% stop-loss / else +0.5% intraday TP or 14:50 time-stop).
